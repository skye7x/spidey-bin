...
SpiderState StateMachine::evaluateTransition(const Vec2& spiderPos,
    ...
    switch (currentState) {
        ...
        case SpiderState::CHARGE:
            if (dist < 30.0f || stateTime > cfg.maxStateDuration)
                return SpiderState::RETREAT;
            // Bug fix: old threshold was alertDist * 2.5f ≈ 1000 px — so far
            // away the spider almost never backed off to STALK gracefully and
            // instead timed out and hard-cut to RETREAT. Reduced to 1.2× for
            // a natural mid-chase transition.
            if (dist > alertDist * 1.2f)
                return SpiderState::STALK;
            break;
