...
public:
    ...
    // Bug fix: removed redundant `currentPos` parameter — Movement owns its
    // position internally; the old round-trip (pass getPosition() in, then
    // immediately do position = currentPos) was fragile and unnecessary.
    void update(float dt, SpiderState state,
